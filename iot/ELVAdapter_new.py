...
class ELV(threading.Thread):
    ...
    def __init__(self, setting, callback=None):
        ...
        self.cmd = {
                    "PM": "In_Service", # In_Service, Out_Service
                    "OP": "Auto", # Auto, Manual
                    "CMD": "Door_Close", # Move_Up, Move_Down, Door_Open, Door_Close, Immediate_Stop, Call_Lifter_Up, Call_Lifter_Down
    ...
    def check_command_completion(self):
        ...
        # if function == "go_floor" and (floor, self.ELV['Status'], self.ELV['OP']) in [(1, "Stopped_DownStair", "Auto"), (2, "Stopped_UpStair", "Auto")]:
        if function == "go_floor" and self.ELV['Status'] == f"Stopped_{floor}F" and self.ELV['OP'] == "Auto":
            ...
            return True
        # elif function == "call_elv" and (floor, self.ELV['Status'], self.ELV['OP']) in [(1, "Stopped_DownStair", "Auto"), (2, "Stopped_UpStair", "Auto")]:
        elif function == "call_elv" and self.ELV['Status'] == f"Stopped_{floor}F" and self.ELV['OP'] == "Auto":
    ...
    def go_floor(self, floor):
        ...
        # if (floor, self.ELV['Status'], self.ELV['OP']) in [(1, "Stopped_DownStair", "Auto"), (2, "Stopped_UpStair", "Auto")]:
        if self.ELV['Status'] == f"Stopped_{floor}F" and self.ELV['OP'] == "Auto":
            self.logger.info('go floor {} check'.format(floor))
            # self.cmd_executing = False
            return True
    ...
    def call_elv(self, floor):
        ...
        if not self.cmd_executing:
            ...
            self.cmd['CMD'] = f"Call_Lifter_{floor}F"
            data = json.dumps(self.cmd_parse.format(**self.cmd))
            self.logger.debug('[send] ' + data)
            self.safe_send(data)
            self.lock.release()
