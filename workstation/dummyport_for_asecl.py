...
from global_variables import Erack #240729
import tools #240729
...
class DummyPortAsecl(threading.Thread):
    ...
    def enter_unloaded_state(self, event):
        ...
        self.dispatch(self.stage, False) #chocp 2022/1/2
    ...
    def enter_loaded_state(self, event, data={}): #chocp 2022/1/2, chocp 2022/6/7 fix
        ...
        self.notify(event)
        self.dispatch(self.stage, True) #not do replace    
        ...
        try:
            if event == 'alarm_set':
                ...
                if self.state == 'UnLoaded' and self.enter_unloaded_time and (time.time()-self.enter_unloaded_time)>self.check_unloaded_timeout:
                    self.enter_unloaded_state(event)
                '''
                elif self.state == 'Unknown' and self.enter_unknown_time and (time.time()-self.enter_unknown_time)>self.check_unknown_timeout:

                    print( int(time.time()-self.enter_unknown_time), self.check_unknown_timeout)
                    self.enter_unknown_state(event)
                    print('loaded_check_req by {}'.format(self.workstationID))
            ...
            if work['Status'] == 'WAITING':
                ...
                if match:
                    ...
                    if global_variables.TSCSettings.get('Other', {}).get('HoldEnable') == 'yes': #only for RTD mode
                        ...
                        #Hshuo for ASECL L5 240729 start
                        res, rack_id, port_no=tools.rackport_format_parse(work['Location'])
                        if res:
                            ...
                            if res:
                                res= h_eRack.carriers[port_no-1].get('checked', False)
                                if not res:
                                    self.orderMgr.my_lock.acquire()
                                    self.logger.info("erack:{} port_no:{} no checked".format(rack_id,port_no))               
                                    self.enter_unknown_state('Unknown')
                                    self.orderMgr.my_lock.release()
                                    continue
                                #Hshuo for ASECL L5 240729 end 
                                else:
                                    self.orderMgr.my_lock.acquire()
