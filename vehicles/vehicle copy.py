...
class Vehicle(threading.Thread):
    ...
    def update_params(self, setting):
        ...
        if self.model == 'Type_B': #new for >8 slot begin
            ...
            self.bufNum=16
            self.vehicle_onTopBufs=['BUF07', 'BUF08']
        ...
        self.Trloadreq_single_timeout=0
        if global_variables.RackNaming == 36:
            self.Trloadreq_single_timeout=60
        else:
            self.Trloadreq_single_timeout=10
    ...
    def update_dynamic_buffer_mapping(self, idx, state):
        ...
        print("Updated dynamicBufferMapping ", self.dynamicBufferMapping)
    ...
    def abort_tr_cmds_and_actions(self, local_command_id, result_code, result_txt, cause='by alarm', check_link=True): #fix 6
        ...
        for action in self.actions:
            if action['local_tr_cmd']['host_tr_cmd']['uuid'] in local_command_id:
                # if global_variables.RackNaming == 36:#peter 241104
                #     E82.report_event(self.secsgem_e82_h, E82.TransferAbortInitiated, {'CommandID':action['local_tr_cmd']['host_tr_cmd']['uuid']})
    ...
    def enter_acquiring_state(self):
        ...
        if global_variables.TSCSettings.get('Other',{}).get('DisablePort2AddrTable', 'no') == 'yes' and len(PortsTable.mapping.get(target, [])) >= 7:
            ...
            self.adapter.logger.info("current_stop:{}".format(current_stop))
            self.adapter.logger.info("next_stop:{}".format(next_stop))
    ...
    def execute_action(self, force_route=False, force_cost= -1, force_path=[]): #fix 8/20
        self.adapter.logger.debug("force_route1:{}".format(force_route))
        #self.ResultCode=0
        local_tr_cmd=self.action_in_run.get('local_tr_cmd', {})
        uuid=local_tr_cmd.get('uuid', '')
                                    ...
                                    if len(host_tr_cmd) > 1:                        
                                        for local_host_tr_cmd in host_tr_cmd:
                                            ...
                                            if self.check_carrier_type == "yes":
                                                carriertype=local_host_tr_cmd[1]['TransferInfoList'][0].get('CarrierType', '')
                                                buf=tools.allocate_buffer(buf_list,carriertype,self.carriertypedict)
                                                if not buf:
                                                    self.adapter.logger.info('{} {} {}'.format('[{}] '.format(self.id), 'No  CarrierType buffer',local_host_tr_cmd[1].get("uuid","")))
                                                    continue
                                                buf_list.remove(buf)
                                            else:
                                                buf=buf_list.pop(0)
                                        ...
                                        if self.check_carrier_type == "yes":
                                            carriertype=host_tr_cmd[0][1]['TransferInfoList'][0].get('CarrierType', '')
                                            buf=tools.allocate_buffer(buf_list,carriertype,self.carriertypedict)
                                            if not buf:
                                                self.adapter.logger.info('{} {} {}'.format('[{}] '.format(self.id), 'No  CarrierType buffer',host_tr_cmd[0][1].get("uuid","")))
                                                continue
                                            buf_list.remove(buf)
                                        else:
                                            buf=buf_list.pop(0)
    ...
    def exec_charge_cmd(self, station, from_unassigned=True):
        ...
        self.adapter.logger.info("action in charge:{}".format(self.actions))
    ...
    def enroute_append_transfer_allowed(self):#2024/10/29 Yuri
        ...
        num, buf_list=self.buf_available()
        if num and tools.appendTransferJudgment(self.actions,self.adapter.last_point):
            if self.wq and tools.acquire_lock_with_timeout(self.wq.wq_lock,3):
                #self.wq.wq_lock.acquire()
                try:
                    if len(self.wq.tr_point) and point in self.wq.tr_point:
                        ...
                        if self.adapter.wait_stop==2:
                            ...
                            if self.adapter.planner.memory_group:
                                group_list=self.adapter.planner.memory_group.split("|")
                                for group in group_list:
                                    if global_variables.global_vehicles_location[group] == self.id:
                                        global_variables.global_vehicles_location[group]=''
                                        self.adapter.logger.info('{} {} {}'.format('[{}] '.format(self.id), 'clean location 6: ', group))
    ...
    def run(self):
        ...
        end_charge=False
        send_alarm=False
        AgvStateIndex=['Unknown','Removed','Unassigned','Enroute','Parked','Acquiring','Depositing','Pause','Charging','TrLoadReq','TrUnloadReq','TrShiftReq','Exchanging','Waiting','Evacuation','Shifting','Suspend']
        ...
        while not self.thread_stop:
            try:
                ...
                if last_AgvState != self.AgvState or last_AgvSubState != self.AgvSubState or last_stop_flag != self.thread_stop:
                    try:
                        self.adapter.logger.debug('{} {} {} {}'.format('[{}] '.format(self.id), 'Vehicle state1: ', self.AgvState, self.AgvSubState, self.thread_stop, self.thread_stop))
                        E82.report_event(self.secsgem_e82_h,
                ...
                if global_variables.global_generate_routes == False:
                    ...
                    if self.AgvState == 'Removed':
                        ...
                        # if global_variables.RackNaming == 36:
                        #     if self.adapter.last_point not in  ["4160_1"]:
                        #         self.adapter.DoorState=''#peter 240705
                        #     global_variables.k11_ng_fault_port[self.id]=''
                        ...
                        if self.waiting_run:
                            ...
                            try:
                                if self.use_schedule_algo == 'by_fix_order':
                                    ...
                                    fail_tr_cmds_id, actions=schedule_by_lowest_cost.task_generate(self.tr_cmds, self.buf_available, self.adapter.last_point)
                                elif self.bufNum>=12:
                    ...
                    #Parked
                                    ...
                                    (self.tr_assert['TransferPort'] == target or self.tr_assert['SendBy'] == 'by web'): #chocp add 2021/12/21
                                        ...
                                        if getattr(h_workstation, "ip", "") and getattr(h_workstation, "port", ""): #by zhenghao zhou
                                            if global_variables.RackNaming in [16,23,34,54]:
                                                door_opened, err=h_workstation.door_action(local_tr_cmd.get('uuid', ''), True)
                        ...
                        if self.enter_shifting_state_time and time.time()-self.enter_shifting_state_time>self.robot_timeout: #timeout 200sec
                        ...
                        if self.adapter.robot['finished'] == 'InterlockError':
                            ...
                            if carrierID and global_variables.RackNaming == 33:
                            ...
                            try:
                                ...
                                if getattr(h_workstation, 'open_door_assist', False):
                                    ...
                                    if global_variables.RackNaming in [16,23,34,54]:
                                        door_opened, err=h_workstation.door_action(uuid, False)
